...
Both machines need the **same Config Sync plugin version** (Overview shows `Plugin: config-sync 1.2.10`). Incoming/outgoing groups, Include checkboxes, and Resync live in that version. Update with `omarchy plugin update gladimdim.config-sync --yes`, or copy `~/.config/omarchy/plugins/gladimdim.config-sync/` from the machine that already has 1.2.10, then `omarchy restart shell`. Removing the plugin also forgets the linked repo, so a reinstall starts at Connect.
