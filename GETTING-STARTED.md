...
**Machine-local files** stay on this machine: display layout (`hypr/monitors.lua`), the generated night-light schedule (`hypr/hyprsunset.conf`), and anything you list as `machine_local` in `.omarchy-config.json`. Overlay files named `*.local.lua` / `local.conf` are not synced at all. Turn on **Include machine-local files** on the Changes tab only if you mean it.
